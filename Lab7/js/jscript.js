...
function verMismaPassword()
{
	var frm = document.getElementById("registro");
	var pass = frm.elements[2].value;
	var pass2 = frm.elements[3].value;
	
	if(pass === pass2)//Las 2 contraseñas no coinciden
	{
		verTelefono();
	}else
	{	
		var texto = "Las 2 passwords no coinciden";
		document.getElementById('textPassword').innerHTML=texto;
		document.getElementById('textPassword').style.color="red";
	}
}
...
{	
	...
	req.open("POST","http://vlana001sw.esy.es/SistemasWeb/Lab7/php/usuarioSW.php",true);
...
function comprobarPasswordList()
{
	...
	req.open("POST","http://vlana001sw.esy.es/SistemasWeb/Lab7/php/ComprobarContrasenaCliente.php",true);
	req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	//req.setRequestHeader("Content-length", parametrosFormulario.length);
	//req.setRequestHeader("Connection", "close");					
	req.send(parametrosFormulario);
...
{	
	var frm = document.getElementById("registro");
	var ticket = frm.elements[4].value;
...
function comprobarTicketList()
{
	...
	/*req.open("POST","http://localhost/SistemasWeblocal/Lab6/php/ComprobarTicketCliente.php",true);
	req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	req.setRequestHeader("Content-length", parametrosFormulario.length);
	req.setRequestHeader("Connection", "close");					
	req.send(parametrosFormulario);*/
	req.open("GET", "http://vlana001sw.esy.es/SistemasWeb/Lab7/php/ComprobarTicketCliente.php?"+parametrosFormulario, true);
	req.send();
}